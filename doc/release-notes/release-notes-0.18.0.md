Diaz Core version 0.18.0 is now available from:

  <https://diazcore.org/bin/diaz-core-0.18.0/>
...
`/Applications/Diaz-Qt` (on Mac) or `diazd`/`diaz-qt` (on
Linux).
...
Diaz Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Diaz Core on unsupported systems.
...
10.10. Additionally, Diaz Core does not yet change appearance when
...
`contrib/init/diazd.service`, it has been changed to use
`/var/lib/diazd` as the data directory instead of
`~diaz/.diaz`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/diazd` will
exist has enough space (check using `df -h /var/lib/diazd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/diaz/diaz/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://diazcore.org/en/contact/) the Diaz Core
...
The systemd init file (`contrib/init/diazd.service`) has been changed
to use `/var/lib/diazd` as the data directory instead of
`~diaz/.diaz`. This change makes Diaz Core more consistent with
...
`datadir` option in `/etc/diaz/diaz.conf` will have no effect.
...
- A new short [document](https://github.com/diaz/diaz/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/diaz/diaz/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/diaz/diaz/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/diaz/diaz/blob/master/doc/diaz-conf.md)
  about the `diaz.conf` file describes how to use it to configure
  Diaz Core.

- A new document introduces Diaz Core's BIP174 [Partially-Signed
  Diaz Transactions
  (PSBT)](https://github.com/diaz/diaz/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/diaz/diaz/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Diaz-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Diaz Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/diaz/diaz/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/diaz/diaz/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Diaz Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Diaz Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/diaz/diaz/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/diaz/diaz/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart diazd with the `-deprecatedrpc=generate`
...
- A new `diaz-wallet` tool is now distributed alongside Diaz
...
This section describes planned changes to Diaz Core that may affect
other Diaz software and services.

- Since version 0.16.0, Diaz Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Diaz Core 0.20 (expected about
  a year after 0.18), Diaz Core will default to native segwit
  ...
  to bech32 addresses, and if the Diaz Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Diaz Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Diaz Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Diaz Core
...
- A sub-project of Diaz Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Diaz Core.  See their [project
  page](https://github.com/diaz-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Diaz Core's own implementation, although entropy
  gathered by Diaz Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Diaz Core a
...
- On macOS, Diaz Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `diaz_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libdiaz_server.a` (jnewbery)
...
- #13926 [Tools] diaz-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like diazd (MarcoFalke)
...
- #14521 qt, docs: Fix `diaz-qt -version` output formatting (hebasto)
...
- #11625 Add DiazApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libdiaz_server` (laanwj)
...
- #15154 Configure: diaz-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/diaz-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Diaz Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libdiaz_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in diaz-util-test.py (isghe)
...
- #12775 Integration of property based testing into Diaz Core (Christewart)
...
- #12246 Bugfix: Only run diaz-tx tests when diaz-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_diaz` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_diaz` (MarcoFalke)
...
- #14186 diaz-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_diaz.conf` message on startup if conf file exists (leishman)
...
- #12255 Update diaz.service to conform to init.md (dongcarl)
...
- #14497 Add doc/diaz-conf.md (hebasto)
...
- #15354 Add missing `diaz-wallet` tool manpages (MarcoFalke)
...
- #15765 correct diazconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support diaz folder on OSX (JimmyMow)
