# RapidCheck property-based testing for Diaz Core

## Concept

Property-based testing is experimentally being added to Diaz Core with
...
If RapidCheck is installed, Diaz Core will automatically run the
...
3. Configure Diaz Core with RapidCheck.

    `cd` to the directory of your local diaz repository and run
...
4. Build Diaz Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_diaz-crypto_gen.o
    CXX      test/test_diaz-key_properties.o
    ```
...
That's it! You are now running property-based tests in Diaz Core.
