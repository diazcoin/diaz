...
* Update `src/chainparams.cpp` chainTxData with statistics about the transaction count and rate. Use the output of the RPC `getchaintxstats`, see
  [this pull request](https://github.com/diaz/diaz/pull/12270) for an example. Reviewers can verify the results by running `getchaintxstats <window_block_count> <window_last_block_hash>` with the `window_block_count` and `window_last_block_hash` from your output.
...
### Diaz maintainers/release engineers, suggestion for writing release notes
...
### Fetch and create inputs: (first time, or when dependency versions change)
    ...
    pushd ./gitian-builder
    mkdir -p inputs
    wget -P inputs https://diazcore.org/cfields/osslsigncode-Backports-to-1.7.1.patch
...
Create (and optionally verify) the signed Windows binaries:
    ...
    mv build/out/diaz-*win32-setup.exe ../diaz-${VERSION}-win32-setup.exe
    popd
...
diaz-${VERSION}.tar.gz
diaz-${VERSION}-win32-setup.exe
diaz-${VERSION}-win32.zip
...
space *do not upload these to the diazwallet.online server, nor put them in the torrent*.
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the diazwallet.online server
...
people without access to `diazwallet.online` to download the binary distribution.
Also put it into the `optional_magnetlink:` slot in the YAML file for
diazwallet.online (see below for diazwallet.online update instructions).

- Update diazwallet.online version

  - First, check to see if the Diaz.org maintainers have prepared a
    release: https://github.com/diaz-dot-org/diazwallet.online/labels/Core
  ...
  - If they have not prepared a release, follow the Diaz.org release
    instructions: https://github.com/diaz-dot-org/diazwallet.online/blob/master/docs/adding-events-release-notes-and-alerts.md#release-notes
