...
class AvoidReuseTest(DiazTestFramework):
    ...
    def test_fund_send_fund_senddirty(self):
        '''
        Test the same as test_fund_send_fund_send, except send the 10 DIAZ with
        the avoid_reuse flag set to false. This means the 10 DIAZ send should succeed,
        ...
        # listunspent should show 1 total outputs (5 diaz), unused
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_count=0)
        # getbalances should show no used, 5 diaz trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        # node 1 should now have about 5 diaz left (for both cases)
