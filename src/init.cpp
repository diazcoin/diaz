 ...
 */
static const char* DIAZ_PID_FILENAME = "diazd.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", DIAZ_PID_FILENAME)));
}
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", DIAZ_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", DIAZ_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/diazcoin/diaz>";
    const std::string URL_WEBSITE = "<https://diazwallet.online>";
...
/** Sanity checks
 *  Ensure that Diaz is running in a usable environment with all
...
{
    // Make sure only a single Diaz process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", DIAZ_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if diaz is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if diaz is started while in a temporary directory.\n",
