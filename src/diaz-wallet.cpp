...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s diaz-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                                      "diaz-wallet is an offline tool for creating and interacting with Diaz Core wallet files.\n" +
                                      "By default diaz-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n" +
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        tfm::format(std::cerr, "No method provided. Run `diaz-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
