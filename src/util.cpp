...
const char * const DIAZ_PID_FILENAME = "diazd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "diazd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Diaz
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Diaz
    // Mac: ~/Library/Application Support/Diaz
    // Unix: ~/.diaz
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Diaz";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Diaz";
#else
    // Unix
    return pathRet / ".diaz";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Diaz Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Diaz Core") == std::string::npos) {
