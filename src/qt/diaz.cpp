...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. diaz_de.qm (shortcut "de" needs to be defined in diaz.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. diaz_de_DE.qm (shortcut "de_DE" needs to be defined in diaz.qrc)
...
void DiazCore::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(m_node.getWarnings("gui")));
}

void DiazCore::initialize()
{
    try
    {
...
static const char* qt_argv = "diaz-qt";

DiazApplication::DiazApplication(interfaces::Node& node):
...
void DiazApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the DiazApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", DiazGUI::DEFAULT_UIPLATFORM);
...
DiazApplication::~DiazApplication()
{
    if(coreThread)
    {
...
#ifdef ENABLE_WALLET
void DiazApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

void DiazApplication::createOptionsModel(bool resetSettings)
{
...
void DiazApplication::startThread()
{
    ...
    DiazCore *executor = new DiazCore(m_node);
...
void DiazApplication::parameterSetup()
{
...
void DiazApplication::SetPrune(bool prune, bool force) {
     optionsModel->SetPrune(prune, force);
}

void DiazApplication::requestInitialize()
{
...
void DiazApplication::requestShutdown()
{
...
void DiazApplication::shutdownResult()
{
...
WId DiazApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6. Determine availability of data directory and parse diaz.conf
